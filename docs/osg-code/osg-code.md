...
=======================================================
# 2.OpenSceneGraph_SOURCE_DIR 在osg源码中哪里定义的
`OpenSceneGraph_SOURCE_DIR` 是在 **CMake 配置过程中自动定义**的变量，不是直接在 OSG 源码中硬编码的。让我详细解释：
...
```cmake
# OpenSceneGraph/CMakeLists.txt 开头附近
cmake_minimum_required(VERSION 3.1...3.19)
project(OpenSceneGraph)

# 这里会自动设置 OpenSceneGraph_SOURCE_DIR
# 它等于 CMAKE_SOURCE_DIR，即包含顶层CMakeLists.txt的目录
```

### 等效定义：
```cmake
# 这些变量是等价的
set(OpenSceneGraph_SOURCE_DIR ${CMAKE_SOURCE_DIR})
set(OpenSceneGraph_BINARY_DIR ${CMAKE_BINARY_DIR})
```
